...
class SimpleMonitor13(simple_switch_13.SimpleSwitch13):
    ...
    def _monitor(self):
        while True:
            ...
            if self.counter >= 1:
                self.counter = 0
                print(f"\n \n ----the count is {self.counter} \n")

            if self.counter2 >= 1 and len(self.bitInfo_newList)!=0:
                self.counter2 = 0
                self.bitInfo_newList = self.bitInfo[-8:]
                if len(self.bitInfo_newList)>=8:
                    ...
                    for bitdif in self.bitDifference:
                        print(bitdif)
                        if bitdif>100000:
                            print("\n ------ Attack --------- \n")
                        else:
                    ...
                    print(f"the bit differences are {self.bitDifference}")

                print(f"the counter is reset {self.counter2} bytes are {self.bitInfo_newList} \n")
    ...
    def _port_stats_reply_handler(self, ev):
        ...
        for stat in sorted(body, key=attrgetter('port_no')):
            ...
            self.tx_bits = stat.tx_bytes
            self.src_add = 0
            self.bitInfo.append(self.tx_bits)
